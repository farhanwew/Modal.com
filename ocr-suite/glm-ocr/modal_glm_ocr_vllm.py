...
APP_NAME = "glm-ocr-vllm"
MODEL_REPO = "zai-org/GLM-OCR"
SERVED_MODEL_NAME = "glm-ocr"
PORT = 8080
GPU_TYPE = os.environ.get("MODAL_GPU", "L4")
# glmocr's own default is 32 ("lower to reduce 503s on a busy server"); vLLM's
# continuous batching shares one KV-cache pool across requests, so raising
# this doesn't shrink anyone else's context budget (see the SGLang attempt in
# CLAUDE.md for the contrasting case where it does).
MAX_WORKERS = os.environ.get("GLM_OCR_MAX_WORKERS", "16")
HF_CACHE_DIR = "/root/.cache/huggingface"

app = modal.App(APP_NAME)
hf_cache = modal.Volume.from_name("glm-ocr-vllm-hf-cache", create_if_missing=True)


# Starting from "devel" (has nvcc) with build tools preinstalled from the
# start, rather than "runtime" — the SGLang attempt burned three separate
# rebuilds discovering it needed a C compiler, then ninja, then nvcc for its
# JIT-compiled kernels one at a time. vLLM JIT-compiles custom ops too.
image = (
    modal.Image.from_registry("nvidia/cuda:12.8.1-devel-ubuntu22.04", add_python="3.12")
    .apt_install("build-essential", "ninja-build")
    .uv_pip_install(
        "vllm>=0.19.0", "transformers>=5.3.0", "glmocr[selfhosted]",
...
class GLMOCR:
    ...
    def start(self) -> None:
        ...
        # Launch command per GLM-OCR's own README ("Using vLLM" section), minus
        # --speculative-config: vLLM's generic "mtp" method expects
        # model.layers.N.mtp_block.* weight keys, which GLM-OCR's checkpoint
        # doesn't ship (its speculative weights are laid out for SGLang's
        # NEXTN naming instead — confirmed by SGLang loading a
        # GlmOcrForConditionalGenerationNextN module from the same checkpoint
        # without issue). Correctness first; revisit speculative decoding
        # later if vLLM adds GLM-OCR-aware weight mapping.
        ...
        while time.time() < deadline:
            if self.process.poll() is not None:
                raise RuntimeError(f"vllm serve exited with {self.process.returncode}")
            try:
            ...
            raise RuntimeError("vllm serve did not become healthy in 900s")
