...
def make_dataset(template: str,
                 ...
                 sample_size: int = 10,
                 sample_mode: str = "random"  # "top", "bottom", or "random"
                 ) -> List["DatasetEntry"]:
    from repeng import DatasetEntry
    dataset = []

    # --- Select subset of suffixes ---
    if not suffix_list:
        return dataset

    if sample_mode == "random":
        chosen_suffixes = random.sample(suffix_list, min(sample_size, len(suffix_list)))
    elif sample_mode == "top":
        chosen_suffixes = suffix_list[:sample_size]
    elif sample_mode == "bottom":
        chosen_suffixes = suffix_list[-sample_size:]
    else:
        chosen_suffixes = suffix_list  # fallback
