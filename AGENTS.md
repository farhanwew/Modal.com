...
- `ppocr-v6/` — despite the name it needs **no PaddlePaddle** (`engine="transformers"`). Its `/table` route is **known-broken upstream** (returns `KMeans n_clusters ... Got 0`), don't chase it; use `paddleocr-vl/`'s `table` task.
...
- **paddleocr-vl / paddleocr-vl-gguf**: same model (PaddleOCR-VL-1.6), transformers vs llama-server. GGUF path needs `--mmproj`; the Hermas-GGUF pattern is text-only.
