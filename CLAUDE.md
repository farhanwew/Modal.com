...
- **ocr-suite/** — the three document-OCR deployments, merged into one folder behind **one** Gradio client (`gradio_app.py`) with a model dropdown. The apps stay separate Modal scripts because they need different base images and GPUs; only the client is shared. Per-app detail lives in `README.paddleocr-vl.md` / `README.paddleocr-vl-gguf.md` / `README.unlimited-ocr.md`, kept verbatim from the old folders.
  ...
  - `modal_unlimited_ocr.py` (app `unlimited-ocr`) — `baidu/Unlimited-OCR`. `infer()` doesn't yield text, so a background thread + `sys.stdout` redirect captures its token-by-token `print()`s; final clean text is read from the `result.md` it writes. Deliberately per-page `infer()`, not `infer_multi()` — the official Space never calls the latter either. This is why `@modal.concurrent` is unsafe there but fine on the PaddleOCR-VL apps.
  - Performance findings that are easy to re-break, all measured on the GGUF app: **memory/GPU snapshots were removed** — restore took ~42 s against ~13 s to just boot, and the `--load-mode none` they forced (mmap off, required for snapshot round-trip) doubled weight-load time on its own. DocLayout-YOLO loads in a **background thread** because `import doclayout_yolo` is 10.1 s of the 10.4 s step and reading the weights is 0.3 s — nothing to speed up, only somewhere else to put it; `/parse-page` waits on a `threading.Event`. PDF rasterisation goes `pixmap.samples → PIL` directly; via `tobytes("png")` it was 429 ms/page instead of 51 ms. `@modal.concurrent(max_inputs=2)` stops a `/health` poll from spawning a second container next to a warm one, and the shared DocLayout-YOLO instance is lock-guarded because Ultralytics models aren't thread-safe.
